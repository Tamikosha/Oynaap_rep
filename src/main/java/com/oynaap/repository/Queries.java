...
public interface Queries {
    ...
    public static final String SQL_SELECT_BLOGPOST=
            "select prod_id,title,description,author from blogposts where blog_id=?";
    ...
    public static final String SQL_SELECT_AN_IMAGEE=
            "select blog_id,title,description,author,category_id,image from blogpost where blog_id=?";
    ...
    "insert into cart(price,quantity,status,prod_id,username,prod_name) value (?,?,?,?,?,?)";

    public static final String SQL_SELECT_ALL_CART_ITEMS=
    "select cart_id,prod_name,status,quantity,price from cart where username=? and payment_status='np'";
